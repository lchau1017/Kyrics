...
class KaraokeMathTest {
    ...
    fun `calculateCharacterAnimation returns default when before start time`() {
        ...
        assertThat(result).isEqualTo(KaraokeMath.CharacterAnimationState.Default)
    }
    ...
    fun `calculatePulseScale oscillates over duration`() {
        val results =
            (0..1000 step 250).map { time ->
                KaraokeMath.calculatePulseScale(currentTimeMs = time)
            }
    ...
    fun `lerpColor returns start color when fraction is 0`() {
        val result = KaraokeMath.lerpColor(Color.Red, Color.Blue, 0f)
    ...
    fun `lerpColor returns end color when fraction is 1`() {
        val result = KaraokeMath.lerpColor(Color.Red, Color.Blue, 1f)
    ...
    fun `lerpColor interpolates all channels`() {
        ...
        val result = KaraokeMath.lerpColor(start, end, 0.5f)
    ...
    fun `lerpColor preserves alpha interpolation`() {
        ...
        val result = KaraokeMath.lerpColor(transparent, opaque, 0.5f)
    ...
    fun `CharacterAnimationState Default has expected values`() {
        val default = KaraokeMath.CharacterAnimationState.Default
    ...
    fun `CharacterAnimationState equality works correctly`() {
        val state1 = KaraokeMath.CharacterAnimationState(1.5f, Offset(1f, 2f), 3f)
        val state2 = KaraokeMath.CharacterAnimationState(1.5f, Offset(1f, 2f), 3f)
        val state3 = KaraokeMath.CharacterAnimationState(1.6f, Offset(1f, 2f), 3f)
