package com.kyrics.demo.data.datasource

import android.content.Context
import com.kyrics.demo.domain.datasource.LyricsDataSource
import com.kyrics.demo.domain.model.LyricsSource
import com.kyrics.models.KyricsLine
import com.kyrics.parseLyrics
import com.kyrics.parser.ParseResult
import dagger.hilt.android.qualifiers.ApplicationContext
import javax.inject.Inject
import javax.inject.Singleton

/**
 * Provides demo lyrics by loading from asset files and parsing with Kyrics library.
 * Supports TTML and LRC formats.
 */
@Singleton
class DemoLyricsDataSource
    @Inject
    constructor(
        @ApplicationContext private val context: Context,
    ) : LyricsDataSource {

        override suspend fun getLyrics(source: LyricsSource): List<KyricsLine> {
            val fileName = "golden-hour.${source.extension}"
            return try {
                val content = context.assets.open(fileName).bufferedReader().use { it.readText() }
                when (val result = parseLyrics(content)) {
                    is ParseResult.Success -> result.lines
                    is ParseResult.Failure -> emptyList()
                }
            } catch (e: Exception) {
                emptyList()
            }
        }

        override fun getTotalDurationMs(): Long = TOTAL_DURATION_MS

        companion object {
            // Golden hour duration: 2:52 = 172 seconds = 172,000 ms
            const val TOTAL_DURATION_MS = 172_830L
        }
    }
